...
const publishAVideo = asyncHandler(async (req, res) => {
    ...
    if ([title, description, videoPublicId, thumbnailPublicId].some(field => !field || field.trim() === "")) {
    ...
    const video = await Video.aggregate([
        ...
        // },
        {
            ...
            $project: {
                ...
                createdAt: 1,
                updatedAt: 1,
                owner: { // Restructure owner details for a cleaner response
...
const updateVideo = asyncHandler(async (req, res) => {
    ...
    if (!title && !description && !videoFileLocalPath && !thumbnailLocalPath) {
        throw new ApiError(400, "At least one field (title, description, video, or thumbnail) is required to update.");
    }
    ...
    if (videoFileLocalPath) {
        ...
        try {
            ...
            console.error("[UpdateVideo] Error caught during video upload via uploadOnCloudinary:", uploadError);
            throw new ApiError(500, "Failed to upload new video file due to an internal error.");
        }

        if (!newVideoFile || !newVideoFile.url || !newVideoFile.public_id) {
            console.error(`[UpdateVideo] Cloudinary video upload failed or returned incomplete data for path: ${videoFileLocalPath}`);
            throw new ApiError(500, "Error while uploading new video file to Cloudinary (no URL or public_id)");
        }
    ...
    if (thumbnailLocalPath) {
        ...
        if (video.thumbnailPublicId) { // Check if there was an old thumbnail public_id to delete
    ...
    if (!updatedVideo) {
        ...
        throw new ApiError(500, "Something went wrong while updating the video. Video might have been deleted.");
    }
